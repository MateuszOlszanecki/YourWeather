...
switchView.forEach((view) => {
  view.addEventListener("click", () => {
    ...
    if (normalView.style.display === "grid") {
      body.style.background = getComputedStyle(
        document.documentElement
      ).getPropertyValue("--specialViewBackgroundColor");
      cityInput[0].value = "";
      cityInput[1].value = "";
      ...
      body.style.background = getComputedStyle(
        document.documentElement
      ).getPropertyValue("--normalViewBackgroundColor");
      cityInput[0].value = "";
      cityInput[1].value = "";
      normalView.style.display = "grid";
      specialView.style.display = "none";
...
function displayData() {
  ...
  cityDateDisplay[0].innerHTML =
    timeDate.toISOString().slice(8, 10) +
    "." +
  ...
  for (let i = 0; i < 2; i++) {
    ...
    //display sunrise
    infoBlocks[3 + 5 * i].innerHTML = new Date(
      (weather["sunrise"] + weather["timezone"]) * 1000
    )
      .toISOString()
      .slice(11, 16);
    //display sunset
    infoBlocks[4 + 5 * i].innerHTML = new Date(
